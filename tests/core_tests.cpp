...
int main() {
    ...
    assert(std::abs(sagas::fgm_pitch_cents(menu_select.voices[0],0) - -580) < 0.01f);
    assert(std::abs(sagas::fgm_pitch_cents(menu_select.voices[0],12) - 1020) < 0.01f);
    assert(sagas::fgm_pitch_cents(menu_select.voices[0],3)==sagas::fgm_pitch_cents(menu_select.voices[0],0));
    assert(sagas::fgm_pitch_cents(menu_select.voices[0],4)==sagas::fgm_pitch_cents(menu_select.voices[0],6));
    assert(sagas::fgm_pitch_cents(menu_select.voices[0],12)==1020);
    const auto punch=sagas::decode_fgm(assets,37);
    const auto layer=std::find_if(punch.voices.begin(),punch.voices.end(),[](const auto& voice){return voice.wave==11;});
    assert(layer!=punch.voices.end());
    assert(sagas::fgm_pitch_cents(*layer,0)==600);
    assert(sagas::fgm_pitch_cents(*layer,3)==-699);
    assert(sagas::fgm_pitch_cents(*layer,4)==-100);
