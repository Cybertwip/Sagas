...
def extract(decomp, manifest):
    ...
    hits=[]; followups=[]
    for clip in sorted({v for k,v in ids.items() if re.fullmatch(r'FT(?:Mario|Fox|Donkey|Samus|Luigi|Link|Yoshi|Captain|Kirby|Pikachu|Purin|Ness)AnimJab[123]',k)}):
        active={};frame=0;followup=-1
        def close(i):
            if i in active:
